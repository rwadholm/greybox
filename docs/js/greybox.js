...
function findIt(pattern, directory, ext){
  findInFiles.find({'term':pattern,'flags':'ig'},directory, ext).then(function(results) {
    ...
    for (let result in results) {
      ...
      currentRes.innerHTML = `Found <b>${res.matches[0]}</b> ${res.count} times in <a href="#" class="gb-openFinderFile" onClick="window.open(this.textContent,'modal');return false;">${result}</a>`
